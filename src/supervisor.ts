...
export function createSupervisedMgbaClient<T extends SupervisedClient>(
  ...
  return new Proxy(client, {
    get(target, property, receiver) {
      if (property === "tap") return supervisor.tap;
      if (property === "tapMany") return supervisor.tapMany;
      if (property === "hold") return supervisor.hold;
      if (property === "holdMany") return supervisor.holdMany;
...
class MgbaSupervisor {
  ...
  tap = async (button: MgbaButton, signal?: AbortSignal): Promise<string> => {
    ...
    const result = isDirection(normalizedButton)
      ? await this.#client.hold(normalizedButton, DIRECTIONAL_HOLD_DURATION, signal)
      : await this.#client.hold(normalizedButton, NON_DIRECTIONAL_TAP_DURATION, signal);
    this.#intervene({
  ...
  hold = async (
    ...
    const result = await this.#client.hold(normalizedButton, fixedDuration, signal);
    await this.#settle(startFrame, signal);
    return result;
  };
  ...
  #requireButton(button: MgbaButton): MgbaButton {
    if (BUTTON_SET.has(button)) return button;
    this.#intervene({
