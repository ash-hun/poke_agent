...
import type { PokemonDirection, PokemonStateObservation } from "./pokemon-state";
...
export class StuckMemory {
  ...
  recordEvent(
    ...
    this.#pendingMovementAttempt = { action, context, turn };
  }
...
export function formatStuckMemory(
  ...
  if (!memory || memory.failedMovementEdges.length === 0) return "";
...
function movementContextFromState(
  ...
  if (!state || state.readStatus !== "available") return;
  if (state.battle || state.dialogueLike === true || state.menuLike === true) return;
  if (
    state.mapId === null ||
...
function isStationary(before: MovementContext, after: MovementContext): boolean {
  if (before.stateBased || after.stateBased) return before.key === after.key;
...
function movementActionFromToolCall(
  ...
  if (!["mgba_hold", "mgba_tap"].includes(toolName)) return;
...
function extractButtons(input: unknown): string[] {
  ...
  if (typeof record.button === "string") return [record.button];
